...
def select_directory_gui():
    """Use native OS file dialog to select directory containing loss plots"""
    try:
...
def plot_loss_from_filenames(directory=None):
    """
    Plots loss values extracted from image filenames in the samples directory.
    Uses native OS file dialog to select directory and creates ONE figure with TWO subplots:
    Left: Raw loss values
    Right: Sliding window average (last 10 epochs)
    No trend lines, just raw data.
    Expects filenames in format: epoch_X_loss_Y.png
    """
    ...
    # Save combined plot
    plot_filename = f'loss_combined_{short_name}.png'
